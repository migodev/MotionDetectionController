...
| Aktiviere Nachlaufsteuerung						    | boolean | 0            | Ist dieser Schalter aktiviert, wird eine Nachlaufsteuerung aktiviert und das FALSE Signal des Bewegungsmelders nicht mehr interpretiert, sondern die Lampen in dem gewünschten Zeitraum automatisch ausgeschaltet.  |
| Laufzeit in Minuten	                                | integer | 0            | Die Laufzeit in Minuten für die Nachlaufsteuerung |
| Aktion beim Deaktivieren                              | integer | 0            | Hier kann gewählt werden, was passiert wenn der Modus der Instanz deaktiviert wird. Entweder bleiben die Ausgangsvariablen geschaltet oder sie werden direkt ausgeschaltet - Keine Funktion bei Nachlaufsteuerung |
| False-Aktion der Eingangsvariable wenn Bedingungen nicht erfüllt sind | integer | 0             | False Meldungen (z.B. der BWM schaltet wieder aus) können hier akzeptiert werden, obwohl die Bedingungen nicht erfüllt sind. In dem Fall wird false immer ausgeführt, das anschalten nur wenn die Bedingungen erfüllt sind - Keine Funktion bei Nachlaufsteuerung|
